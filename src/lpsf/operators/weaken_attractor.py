...
def weaken_attractor(
    ...
    *,
    ...
    meta = {"path_priority": -strength, "activation_threshold_delta": strength * 0.1}
    if score_delta_meta:
        meta.update(score_delta_meta)
    prepared = prepare_mark(
        conn,
        operator_type="weaken_attractor",
        event_id=event_id,
    ...
    superseded = supersede_active_marks(
        conn,
        operator_types=("deepen_attractor",),
        target_type=target_type,
        target_id=target_id,
        scope=scope,
        snapshot_id=snapshot_id,
    ...
    state_delta = upsert_attractor(
        ...
        decay_state_extra={"weakening_reason": reason},
    )
